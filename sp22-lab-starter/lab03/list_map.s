...
main:
    ...
    add a0, s0, x0 # passing the s0 node to function argument


    la a1 square # a1 as a argument to "map"
...
map:
    ...
    # THINK: Why a0?
    ### YOUR CODE HERE ###
    lw a0 0(s0) # load the current node value into a0

    # Call the function in question on that value. DO NOT use a label (be prepared to answer why).
    # Hint: Where do we keep track of the function to call? Recall the parameters of "map".
    ### YOUR CODE HERE ### # 
    # //because the sub-function f only need one argument a0 then a1 need not to change//

    jalr s1 # jump to the function 
    ...
    ### YOUR CODE HERE ###
    sw a0 0(s0)  # store the result to the node
    # Load the address of the next node into a0
    # The address of the next node is an attribute of the current node.
    # Think about how structs are organized in memory.
    ### YOUR CODE HERE ###
    lw a0 4(s0) # load the next node to the a0
    # Put the address of the function back into a1 to prepare for the recursion
    # THINK: why a1? What about a0?
    ### YOUR CODE HERE ###
    add a1 s1 x0  # the address of funciton
    
    # Recurse
    ### YOUR CODE HERE ###
    # when function call itself the ra value didnot change????
    jal ra map
